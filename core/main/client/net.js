...
beef.net = {
     ...
     *  - allowCrossDomain: set cross-origin requests as allowed or blocked
     *
    ...
    forge_request: function (scheme, method, domain, port, path, anchor, headers, data, timeout, dataType, allowCrossDomain, requestid, callback) {
        ...
        var cross_origin = true;
        if (document.origin == origin && document.location.protocol == scheme + ':') {
            if (document.location.port == "" || document.location.port == null) {
                cross_origin = !(port == "80" || port == "443");
            } else {
        ...
        if (allowCrossDomain == "false" && cross_origin) {
